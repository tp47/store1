...
class App extends Component {
  ...
  componentWillMount() {
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
        ...
        if (user.email.includes("@admin.com")) isAdminTemp = true;
        // User is signed in.
        ...
        this.setState({
          ...
          currentUserInfo: currentUserInfoTemp,
        });
        toast.success(`Добро пожаловать, ${this.state.currentUserInfo.email}`);
      } else {
  ...
  getProduct = (id) => {
    let productTemp = this.state.products.filter(
  ...
  addToCart = (id) => {
    ...
    this.setState(
      (preState) => {
        return {
          cart: [productTemp, ...preState.cart],
  ...
  inCartHandler = (id, type) => {
    ...
    if (type === true) productTemp.inCart = true;
    else productTemp.inCart = false;
  ...
  productTotalPriceHandler = (id, type) => {
    ...
    if (type === "increase") productTemp.total += productTemp.price;
    else productTemp.total -= productTemp.price;
  ...
  searchingHandler = (name) => {
    this.setState((preState) => ({
      filteredProducts: preState.products.filter(
        (product) =>
  ...
  modalInfoHandler = (id) => {
    let modalInfoTemp = this.state.products.filter(
      (product) => product.id === id,
    )[0];
  ...
  productsPriceFilter = (event, newValue) => {
    ...
    if (min === 0 && max === 0)
      ...
      filteredProductsTemp = this.state.products.filter(
  ...
  forgetPasswordHandler = (email, props) => {
    firebase
      ...
      .then(() => {
        ...
        toast.success("Email для восстановления отправлен!", {
          position: "top-center",
          autoClose: 4000,
          hideProgressBar: false,
          closeOnClick: true,
      ...
      <div className="App">
        ...
        <Switch>
          ...
          <Route
            ...
            render={(props) => (
              <ProductDetails
                productDetails={this.state.products}
                addToCart={this.addToCart}
                {...props}
              />
            )}
          ...
          <Route
            ...
            render={(props) => {
              return isAuthenticated ? (
                ...
                <Register registerHandler={this.registerHandler} {...props} />
              );
            }}
          ></Route>
          <Route
